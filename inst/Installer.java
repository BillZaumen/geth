...
public class Installer {
	...
	if (windowsOS) {
		...
		JOptionPane.showMessageDialog(null,
					      ...
					      + "missing",
					      "gethdrs Installer Error",
					      JOptionPane.ERROR_MESSAGE);
		System.exit(1);
	    } else {
		wdirname = "%PROGRAMDATA%"
		    + fs.getSeparator() + "gethdrs";
	    }
	    ...
	    if (Files.isDirectory(fs.getPath("/usr/local/share/gethdrs"))) {
		ulButton.setSelected(true);
		Path p =
		    fs.getPath("/usr/local/share/gethdrs/man/man1/lsnof.1.gz");
		if (Files.exists(p)) {
		    gzipManCheckBox.setSelected(true);
		}
	    } else if (Files.isDirectory(fs.getPath("/opt/gethdrs"))) {
		...
		Path p =
		    fs.getPath("/opt/gethdrs/man/man1/lsnof.1.gz");
		if (Files.exists(p)) {
		    gzipManCheckBox.setSelected(true);
		}
	    } else {
	...
	runButton.addActionListener(new AbstractAction() {
		public void actionPerformed(ActionEvent event) {
			...
			if (!isWindows && ulButton.isSelected()) {
				...
				(fs.getPath("/usr/local/share/gethdrs"))) {
			...
			if (!isWindows
			    ...
			    if (!Files.isDirectory(fs.getPath("/opt/gethdrs"))) {
				JOptionPane.showMessageDialog
				    (frame,
			...
			JOptionPane.showMessageDialog
			    (frame, tf, "gethdrs Installer Java Command",
			...
			cleanupOnError();
			JOptionPane.showMessageDialog
			    (frame, e.getMessage(), "gethdrs Installer Error",
			     JOptionPane.ERROR_MESSAGE);
			System.exit(1);
		    } catch(Error e) {
			cleanupOnError();
	...
	if (usrlocalMode) {
	    ...
	    cvrdir = cvrdir.resolve("gethdrs");
	} else {
	    bzdir = installPath.resolve("bzdev");
	    cvrdir = installPath.resolve("gethdrs");
	}
	if (installPathString != null) {
	    cvrdirString = installPathString
		+ System.getProperty("file.separator") + "gethdrs";
	}
	Path ourOld =  cvrdir.resolveSibling("gethdrs-old");
	...
	if (uninstall || (Files.isDirectory(cvrdir) && Files.exists(tspath))) {
		...
		if (Files.isDirectory(cvrdir)) {
		    ...
		    if (stdman5 != null && Files.isDirectory(stdman5)) {
			for (String name: man5s) {
			    Path p = stdman5.resolve(name);
			    if (Files.isSymbolicLink(p)) {
				// make sure the symbolic link points to
				// a file in our man1 directory.
				Path dp = p.resolveSibling
	...
	if (posixMode) {
		...
		file = file.replace("GETHDIR", cvrdir.toString());
		...
		if (cvrdirString != null) {
		    file = file.replace("GETHDIR", cvrdirString);
		} else {
		    file = file.replace("GETHDIR", "\"" + cvrdir.toString()
					+ "\"");
		}
	...
	if (posixMode) {
		...
		if (stdman5 != null) {
		    for (String name: man5sGzip) {
			Path link = stdman5.resolve(fs.getPath(name));
			Path target = cvrman5.resolve(fs.getPath(name));
			Path rtarget = link.getParent().relativize(target);
			if (Files.isSymbolicLink(link)) {
			    Path tlink = Files.readSymbolicLink(link);
			    if (!tlink.equals(rtarget)) {
		...
		for (String name: man5s) {
		    ...
		    createFile(cvrman5, fs.getPath(name), dirPerm, filePerm,
			       is);
		    is.close();
		}
		if (stdman5 != null) {
